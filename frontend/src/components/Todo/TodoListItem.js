...
const TodoListItem = ({
  ...
  const [editedMessage, setEditedMessage] = useState(message.task); // State to hold the edited task string
  ...
  const handleKeyEscape = (event) => {
    if (event.key === "Escape") {
      setEditedMessage(message.task); // reset to original task string
      toggleEditMode();
    }
  };
